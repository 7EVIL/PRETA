...
bool UInventoryComponent::CanCombineItemsObjects(UItem* FirstItem, UItem* SecondItem)
{
	if (!IsValid(CombinesDataTable)) return false;
	...
	TSubclassOf<UItem> FirstItemClass = FirstItem->GetClass();
	TSubclassOf<UItem> SecondItemClass = SecondItem->GetClass();
	if (FirstItem == SecondItem && FirstItem->Amount <=1)
	{
		return false;
	}
	...
	for (const FCombinesStruct* Row : AllRows)
	{
			...
			SecondItem == ItemOneClass)
		{
			...
			if (!FindItemToStack(Row->Result, ESlotsType::Primary, CanAdd, ItemToStack))
			{
				...
				if (ItemTwo->Amount == 1 || (ItemOne == ItemTwo && ItemOne->Amount == 2))
					ExcludeSlots.Append(ItemTwo->OccupiedSlots);

				if (ItemOne == ItemTwo && ItemOne->Amount <= 1)
				{
